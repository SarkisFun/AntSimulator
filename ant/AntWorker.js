...
export class AntWorker {
    ...
    static canvas;
      ...
      draw(rotationAngle) {
        AntWorker.ctx.save();
        AntWorker.ctx.translate(this.posX, this.posY);
        AntWorker.ctx.rotate(rotationAngle + Math.PI / 2);
        if (AntWorker.img.complete) {
            ...
            AntWorker.ctx.drawImage(AntWorker.img, 0, 0, AntWorker.scale, AntWorker.scale);
        } else {
      ...
      update() {
        ...
        if (this.posX - this.size < 0 || this.posX + this.size > AntWorker.canvas.width) {
          //dx *= -1;
          this.angle = Math.PI - this.angle;
          this.posX = Math.max(0, Math.min(AntWorker.canvas.width, this.posX));
        }
        if (this.posY - this.size < 0 || this.posY + this.size > AntWorker.canvas.height) {
          //dy *= -1;
          this.angle = -this.angle;
          this.posY = Math.max(0, Math.min(AntWorker.canvas.height, this.posY));
        } 
